...
\begin{itemize}
	...
	\item Get angular rate data for $v_z$: 1 write transaction (angular rate data address) + 2 bytes returned for the data = 4 bytes
	
	\item Get angular rate data for $v_y$: 4 bytes
	
	\item Get angular rate data for $v_x$: 4 bytes
...
\begin{equation}
4 \times 3 + 4 \times 3 + 14 = 38 \ bytes
\end{equation}

So the time required for the data transfer is:

\begin{equation}
38 * 8 / 400 \times 10^3 = 760 \ [\mu s]
\end{equation}
